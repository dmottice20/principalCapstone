...
def perc_change(df, feature, time_lens):
    ...
    for time in time_lens:
        ...
        for i, row in df.iterrows():
            if (i - relativedelta(months=+time)) in df.index:
                perc_change_list.append((df[feature][i] - df[feature][i - relativedelta(months=+time)])
                                        / df[feature][i - relativedelta(months=+time)])
            elif (i - relativedelta(months=+time) - relativedelta(days=+1)) in df.index:
                perc_change_list.append(
                    (df[feature][i] - df[feature][i - relativedelta(months=+time) - relativedelta(days=+1)])
                    / df[feature][i - relativedelta(months=+time) - relativedelta(days=+1)])
            elif (i - relativedelta(months=+time) - relativedelta(days=+2)) in df.index:
                perc_change_list.append(
                    (df[feature][i] - df[feature][i - relativedelta(months=+time) - relativedelta(days=+2)])
                    / df[feature][i - relativedelta(months=+time) - relativedelta(days=+2)])
            elif (i - relativedelta(months=+time) - relativedelta(days=+3)) in df.index:
                perc_change_list.append(
                    (df[feature][i] - df[feature][i - relativedelta(months=+time) - relativedelta(days=+3)])
                    / df[feature][i - relativedelta(months=+time) - relativedelta(days=+3)])
            else:
...
"""
"""
US_Returns = pd.read_excel('US IG and US HY Returns.xlsx', 'HYCorp')
US_Returns['Value Date'] = pd.to_datetime(US_Returns['Value Date'])
US_Returns.set_index('Value Date', inplace=True)
"""
# print(monthly_prev_return(US_Returns, 'Excess Return', [1]))
# merged_data = pd.read_csv('merged_data.csv')


# look at percent change for 1m and 3m
# monthly_fut_return(return_data, 'HY Excess Return', [1, 3])

# print(return_data.head(150))
...
featured_daily_data = daily_data[['1m variance', '3m variance']]
...
train_X, test_X, train_y, test_y = train_test_split_USAFA(input_data=merged_data_eom[['1m variance',
                                                                                     '3m variance']],
                                                          response_data=merged_data_eom['fut_1m_return'])
...
print('# of features used', coeff_used)
